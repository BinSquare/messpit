...
async fn get_regions(
    ...
    // Use platform abstraction to get regions (works on all platforms)
...
async fn start_scan(request: ScanRequest, state: State<'_, AppState>) -> Result<Vec<ScanResult>, String> {
    ...
    // Use platform abstraction for memory access (works on all platforms)
    let all_raw_results: Vec<(Address, Value)> = {
        let guard = state.attached.lock_checked()?;
        let attached = guard.as_ref().ok_or_else(|| error_requires("an attached process"))?;

        // Get regions using platform abstraction
        let regions = attached.handle.regions()
            .map_err(|e| format!("Failed to get regions: {}", e))?;
        ...
        for region in regions {
            ...
            if region.size as usize > MAX_REGION_READ_SIZE {
                continue;
            }

            // Read region memory using platform abstraction
            let mut buffer = vec![0u8; region.size as usize];
            if attached.handle.read_memory(region.base, &mut buffer).is_ok() {
...
async fn refine_scan(request: RefineRequest, state: State<'_, AppState>) -> Result<Vec<ScanResult>, String> {
    // Get previous results and value type
    ...
    let refined = {
        ...
        // Perform refinement using platform abstraction
        ScanEngine::refine_scan(
            &previous,
            |addr, sz| {
                let mut buf = vec![0u8; sz];
                if attached.handle.read_memory(addr, &mut buf).is_ok() {
                    Some(buf)
                } else {
                    None
                }
...
async fn get_watches(state: State<'_, AppState>) -> Result<Vec<WatchInfo>, String> {
    ...
    let watches = {
        ...
        session.watches().map(|w| {
            let frozen = session.freezes.contains_key(&w.id);
            let freeze_value = session.freezes.get(&w.id).map(|f| format_val(&f.value));

            // Read current value using platform abstraction
            let current_value = if let Some(ref attached) = *attached_guard {
...
async fn read_value(request: ReadValueRequest, state: State<'_, AppState>) -> Result<Option<String>, String> {
    ...
    if attached.handle.read_memory(address, &mut buf).is_ok() {
        Ok(decode_at(&buf, &value_type).map(|v| format_val(&v)))
    } else {
        Ok(None)
    }
}
...
async fn read_memory_bytes(
    ...
    let mut buf = vec![0u8; size];
    attached.handle.read_memory(addr, &mut buf)
...
async fn pattern_scan(request: PatternScanRequest, state: State<'_, AppState>) -> Result<Vec<PatternScanResult>, String> {
    ...
    // Use platform abstraction for memory access (works on all platforms)
    let result_list: Vec<PatternScanResult> = {
        let guard = state.attached.lock_checked()?;
        let attached = guard.as_ref().ok_or_else(|| error_requires("an attached process"))?;
        ...
        let scan_regions: Vec<_> = if let Some(ref module_name) = module_filter {
            ...
            regions.into_iter()
                .filter(|r| {
                    r.permissions.read && r.module.as_ref()
            ...
            if attached.handle.read_memory(region.base, &mut buffer).is_ok() {
                ...
                for offset in 0..=scan_end {
                    if pattern.matches_at(&buffer, offset) {
                        let address = region.base.0 + offset as u64;
                        let module_offset = region.module.as_ref().map(|_| offset as u64);
...
async fn resolve_pointer_chain(request: ResolveChainRequest, state: State<'_, AppState>) -> Result<String, String> {
    ...
    let resolved = {
        ...
        // Find module base address using platform abstraction
        let module_base = if let Some(ref module_name) = request.module {
            let module_lower = module_name.to_lowercase();
            let modules = attached.handle.modules().unwrap_or_default();
            modules.into_iter()
                .find(|m| m.name.to_lowercase().contains(&module_lower) ||
                         m.path.as_ref().is_some_and(|p| p.to_lowercase().contains(&module_lower)))
                .map(|m| m.base.0)
        ...
        for (i, offset) in chain.offsets.iter().enumerate() {
            // Read pointer at current address using platform abstraction
            let mut ptr_buf = vec![0u8; pointer_size];
            attached.handle.read_memory(Address(current_addr), &mut ptr_buf)
...
async fn get_signatures(state: State<'_, AppState>) -> Result<Vec<SignatureInfo>, String> {
    // Get signature data (resolution is done on-demand via resolve_signature)
    let project = state.project.lock_checked()?;
    ...
    let sigs: Vec<SignatureInfo> = project.signatures.iter().map(|sig| {
        SignatureInfo {
            ...
            resolved_address: None, // Resolution is done on-demand
        }
    }).collect();
...
async fn resolve_signature(sig_id: String, state: State<'_, AppState>) -> Result<Option<String>, String> {
    // Get signature data
    let (pattern_str, module, offset) = {
        let project = state.project.lock_checked()?;
        let sig = project.get_signature(&sig_id)
            .ok_or("Signature not found")?;
        (sig.pattern.clone(), sig.module.clone(), sig.offset)
    };
    ...
    let address: Option<Address> = {
        let guard = state.attached.lock_checked()?;
        let attached = guard.as_ref().ok_or("No process attached")?;
        ...
        let scan_regions: Vec<_> = regions.into_iter()
            .filter(|r| {
                r.permissions.read && r.permissions.execute &&
                r.module.as_ref().is_some_and(|m| m.to_lowercase().contains(&module_lower))
            })
            .collect();
            ...
            if attached.handle.read_memory(region.base, &mut buffer).is_ok() {
                ...
                for match_offset in 0..=scan_end {
                    if pattern.matches_at(&buffer, match_offset) {
                        let address = region.base.0 + match_offset as u64;
                        result = Some(Address((address as i64 + offset) as u64));
                        break;
                    }
                }
            }
...
async fn watch_from_signature(sig_id: String, state: State<'_, AppState>) -> Result<String, String> {
    // Get signature data
    let (sig_label, sig_pattern, sig_module, sig_offset, sig_value_type) = {
        let project = state.project.lock_checked()?;
        let sig = project.get_signature(&sig_id)
            .ok_or("Signature not found")?;
        (
    ...
    let address = {
        ...
        for region in scan_regions {
            ...
            if attached.handle.read_memory(region.base, &mut buffer).is_ok() {
                ...
                for match_offset in 0..=scan_end {
                    if pattern.matches_at(&buffer, match_offset) {
                        let addr = region.base.0 + match_offset as u64;
                        result = Some(Address((addr as i64 + sig_offset) as u64));
                        break;
                    }
                }
            }
        ...
        result.ok_or("Failed to resolve signature")?
    };
